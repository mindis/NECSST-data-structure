...
void* art_search(const art_tree *t, const unsigned char *key, int key_len) {
    ...
    while (n) {
        ...
        if (n->partial_len) {
		/*	
			if ((n->partial_len - parent_len) < 0) {
				parent_len = 0;
			}
			*/
			
            depth = depth + n->partial_len - parent_len;
			if (n->split_type == SPLIT) {
				parent_len = n->partial_len + 1;
			} else if (n->split_type == NO_SPLIT) {
				parent_len = 0;
			}
			...
			*/
		} else if (n->split_type == SPLIT && n->history == key[depth]) {
			parent_len = 1;
		}
		else if (n->split_type == NO_SPLIT) {
			parent_len = 0;
...
//			parent_len = 0;
		...
		if (n->split_type == SPLIT && n->history != key[depth]) {
			parent_len = 0;
		}
        // Recursively search
...
static void copy_header(art_node *dest, art_node *src) {
	...
	dest->split_type = src->split_type;
...
static void* recursive_insert(art_node *n, art_node **ref, const unsigned char *key, int key_len, void *value, int depth, int *old, int *parent_len, uint8_t split_type) {
    ...
    if (IS_LEAF(n)) {
		...
		new_node->n.split_type = NO_SPLIT;
		new_node->n.history = 0;
    ...
    if (n->partial_len) {
        ...
        //int prefix_diff = prefix_mismatch(n, key, key_len, 0, &l);
		...
		*/
		...
		if ((uint32_t)prefix_diff >= n->partial_len - *parent_len) {
            depth += n->partial_len - *parent_len;
			if (n->split_type == SPLIT) {
				*parent_len = n->partial_len + 1;
			} else if (n->split_type == NO_SPLIT) {
				*parent_len = 0;
 		...
 		/*
		if (split_type == SPLIT && n->split_type = SPLIT) {
			depth = 0;
			prefix_diff = prefix_mismatch(n, key, key_len, depth, &l);
		}
		*/
...
//		printf("parent len = %d\n", *parent_len);
        ...
        *ref = (art_node*)new_node;
		if (split_type == SPLIT && n->split_type == SPLIT) {
			printf("hello\n");
			new_node->n.partial_len = prefix_diff + *parent_len;
		} else {
       		new_node->n.partial_len = prefix_diff;
		}
		new_node->n.split_type = SPLIT;
        add_child4(new_node, ref, l->key[depth+prefix_diff], n);
		new_node->n.history = l->key[depth + prefix_diff];
...
RECURSE_SEARCH:;
    ...
    if (child) {
        return recursive_insert(*child, child, key, key_len, value, depth+1, old, parent_len, n->split_type);
    }
...
void* art_insert(art_tree *t, const unsigned char *key, int key_len, void *value) {
    int old_val = 0;
	int parent_len = 0;
    void *old = recursive_insert(t->root, &t->root, key, key_len, value, 0, &old_val, &parent_len, 2);
    if (!old_val) t->size++;
    return old;
}
...
int art_iter_prefix(art_tree *t, const unsigned char *key, int key_len, art_callback cb, void *data) {
    ...
    while (n) {
        ...
        if (n->partial_len) {
			art_leaf *l;
            prefix_len = prefix_mismatch(n, key, key_len, depth, &l);
