...
static void* recursive_insert(art_node *n, art_node **ref, const unsigned char *key, int key_len, void *value, int depth, int *old, int *parent_len) {
...
RECURSE_SEARCH:;
    ...
    if (child) {
        return recursive_insert(*child, child, key, key_len, value, depth+1, old, parent_len);
    }
...
void* art_insert(art_tree *t, const unsigned char *key, int key_len, void *value) {
    int old_val = 0;
	int parent_len = 0;
    void *old = recursive_insert(t->root, &t->root, key, key_len, value, 0, &old_val, &parent_len);
    if (!old_val) t->size++;
    return old;
}
