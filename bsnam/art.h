...
typedef struct {
    uint8_t type;           // 8
    uint8_t num_children;   // 8
    uint32_t partial_len;   // 32
    unsigned char partial[MAX_PREFIX_LEN];  // 16
} art_node;   // 64 bytes

typedef struct {
    art_node *children[32];   // 256 bytes
} child_ptr_node;

typedef struct {
    art_node n;                   // 64 bytes
    unsigned char keys[256];      // 256 bytes
    child_ptr_node* ptr_node[8];  // 64 bytes
} linked_node;
