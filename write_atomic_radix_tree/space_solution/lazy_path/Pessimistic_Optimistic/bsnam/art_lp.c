...
unsigned long child_node_count = 0;
...
static art_node* alloc_node() {
	...
	memset(((art_node16 *)n)->keys, -1, sizeof(((art_node16 *)n)->keys));
	node_count++;
	return n;
}
...
static art_node** find_child(art_node *n, unsigned char c) {
	...
	pos = p->keys[c];
	if (pos != -1) {
		if (p->child_group[pos/32]) {
			if ((p->child_group[pos/32])->children[pos%32])
				return &(p->child_group[pos/32])->children[pos%32];
		}
	}
...
static art_leaf* minimum(const art_node *n) {
	...
	for (i = 0; i < 8; i++) {
		if (((art_node16 *)n)->child_group[i]) {
			for (j = 0; j < 32; j++) {
				if ((((art_node16 *)n)->child_group[i])->children[j])
					return minimum((((art_node16 *)n)->child_group[i])->children[j]);
			}
		}
	}
	return NULL;
...
static void add_child_noflush(art_node16 *n, art_node **ref, unsigned char c, void *child) {
	int p_pos, c_pos;
	p_pos = n->n.num_children / 32;

	if (n->child_group[p_pos] == NULL) {
		n->child_group[p_pos] = (child_node *)calloc(1, sizeof(child_node));
		child_node_count++;
		for (c_pos = 0; c_pos < 32; c_pos++) {
			if ((n->child_group[p_pos])->children[c_pos] == NULL) {
				n->keys[c] = p_pos * 32 + c_pos;
				(n->child_group[p_pos])->children[c_pos] = child;
				n->n.num_children++;
				break;
			}
		}
...
static void add_child(art_node16 *n, art_node **ref, unsigned char c, void *child) {
	...
	if (n->child_group[p_pos] == NULL) {
		...
		flush_buffer(n->child_group[p_pos], sizeof(child_node), false);
		flush_buffer(&n->child_group[p_pos], 8, true);
	} else {
		for (c_pos = 0; c_pos < 32; c_pos++) {
			if ((n->child_group[p_pos])->children[c_pos] == NULL) {
				n->keys[c] = p_pos * 32 + c_pos;
		...
		flush_buffer(&(n->child_group[p_pos])->children[c_pos], 8, true);
	}
}
...
//		memcpy(new_node->n.partial, key+depth, min(MAX_PREFIX_LEN, longest_prefix));
		...
		*ref = (art_node*)new_node;
		add_child_noflush(new_node, ref, get_index(l->key, depth + longest_prefix), SET_LEAF(l));
		add_child_noflush(new_node, ref, get_index(l2->key, depth + longest_prefix), SET_LEAF(l2));

		flush_buffer(new_node, sizeof(art_node16), false);
		flush_buffer(l2, sizeof(art_leaf), false);
		flush_buffer(new_node->child_group[0], sizeof(child_node), false);
		flush_buffer(ref, 8, true);
		return NULL;
	}
...
//			add_child4(new_node, ref, n->partial[prefix_diff], n);
			add_child_noflush(new_node, ref, copy_node->partial[prefix_diff], copy_node);
			...
			if (l == NULL)
				l = minimum(copy_node);
			add_child_noflush(new_node, ref, get_index(l->key, depth + prefix_diff), copy_node);
...
//					min(MAX_PREFIX_LEN, copy_node->partial_len));
		...
		add_child_noflush(new_node, ref, get_index(key, depth + prefix_diff), SET_LEAF(l));
