 ...
 */
#define IS_LEAF(x) (((uintptr_t)x & 1))		//check is leaf?
#define SET_LEAF(x) ((void*)((uintptr_t)x | 1))		//leaf type set
#define LEAF_RAW(x) ((art_leaf*)((void*)((uintptr_t)x & ~1)))	//leaf type 해제
...
static void add_child256_nochange(art_node256 *n, art_node **ref, unsigned char c, void *child) {
	(void)ref;
	n->n.num_children++;
...
static void add_child16(art_node16 *n, art_node **ref, unsigned char c, void *child) {
	if (n->n.num_children < 16) {
		...
		flush_buffer(&n->keys[n->n.num_children], sizeof(unsigned char), false);
		flush_buffer(&n->children[n->n.num_children], 8, false);
		...
		for (i=0;i<16;i++) {
			if (n->keys[i]) {
				new_node->children[n->keys[i]] = n->children[i];
			}
		}
		copy_header((art_node*)new_node, (art_node*)n);
		*ref = (art_node*)new_node;
...
static void add_child4_nochange(art_node4 *n, art_node **ref, unsigned char c, void *child) {
	// Insert element
	n->keys[n->n.num_children] = c;
	n->children[n->n.num_children] = (art_node*)child;
	n->n.num_children++;
...
static void add_child(art_node *n, art_node **ref, unsigned char c, void *child) {
	switch (n->type) {
		...
		case NODE256:
			return add_child256_nochange((art_node256*)n, ref, c, child);
		default:
			abort();
	}
}
...
static void* recursive_insert(art_node *n, art_node **ref, const unsigned char *key, int key_len, void *value, int depth, int *old) {
	...
	if (IS_LEAF(n)) {
		...
		*ref = (art_node*)new_node;
		add_child4_nochange(new_node, ref, l->key[depth+longest_prefix], SET_LEAF(l));
		add_child4_nochange(new_node, ref, l2->key[depth+longest_prefix], SET_LEAF(l2));
	...
	if (n->partial_len) {
		...
		add_log_entry(start_log, n, sizeof(art_node), LE_DATA);
		...
		*ref = (art_node*)new_node;
		...
		if (n->partial_len <= MAX_PREFIX_LEN) {
			add_child4_nochange(new_node, ref, n->partial[prefix_diff], n);
			...
			n->partial_len -= (prefix_diff+1);
			art_leaf *l = minimum(n);
			add_child4_nochange(new_node, ref, l->key[depth+prefix_diff], n);
		...
		add_child4_nochange(new_node, ref, key[depth+prefix_diff], SET_LEAF(l));
